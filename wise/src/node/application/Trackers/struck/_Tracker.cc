...
_Tracker::_Tracker(const Config& conf) :
	m_config(conf),
	m_initialised(false),
	m_pLearner(0),
...
void _Tracker::Initialise(const cv::Mat& frame, FloatRect bb)
{
	m_bb = IntRect(bb);
...
void _Tracker::Track(const cv::Mat& frame)
{
...
void _Tracker::UpdateDebugImage(const vector<FloatRect>& samples, const FloatRect& centre, const vector<double>& scores)
{
...
void _Tracker::Debug()
{
	imshow("_Tracker", m_debugImage);
	m_pLearner->Debug();
}

void _Tracker::UpdateLearner(const ImageRep& image)
{
