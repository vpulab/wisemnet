...
public:
	...
	_Rect(T xMin, T yMin, T width, T height) :
		m_xMin(xMin),
		m_yMin(yMin),
		m_width(width),
		m_height(height)
	...
	template <typename T2>
	friend std::ostream& operator <<(std::ostream &rOS, const _Rect<T2>& rRect);
	
	template <typename T2>
	float Overlap(const _Rect<T2>& rOther) const;
	
	template <typename T2>
	bool IsInside(const _Rect<T2>& rOther) const;
...
typedef _Rect<int> IntRect;
typedef _Rect<float> FloatRect;

template <typename T>
std::ostream& operator <<(std::ostream &rOS, const _Rect<T>& rRect)
{
...
template <typename T>
template <typename T2>
float _Rect<T>::Overlap(const _Rect<T2>& rOther) const
{
...
template <typename T>
template <typename T2>
bool _Rect<T>::IsInside(const _Rect<T2>& rOther) const
{
