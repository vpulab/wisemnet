...
//		imgFormat = conf.sequenceBasePath+"/"+conf.sequenceName+"/imgs/img%05d.png";
		imgFormat = conf.sequenceBasePath+"/"+conf.sequenceName+"/imgs/%0d.jpg";
		...
		string outputFile = conf.resultsPath + ".avi";
		Size S = cvSize(conf.frameWidth, conf.frameHeight);
		outputVideo.open(outputFile, CV_FOURCC('D','I','V','X'), 20, S, true);
	...
	for (int frameInd = startFrame; frameInd <= endFrame; ++frameInd)
	{
		...
		{			
			...
			if (frameInd == startFrame)
			{
				...
				Mat tmp = frame;
				rectangle(tmp, initBB, CV_RGB(0, 255, 0));
				imshow("Initial target",tmp);
				waitKey(0);
				destroyWindow("Initial target");
			}
		}
