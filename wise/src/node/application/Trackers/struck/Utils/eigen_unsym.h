// Taken from "Numeric recipes"

typedef int Int;
typedef double Doub;
typedef double** MatDoub;
typedef double* VecComplex;
typedef double* VecDoub;
typedef int* VecInt;
typedef bool Bool;
...
struct Unsymmeig {
	...
	Unsymmeig(const Mat &aa, bool vec) :
		n(aa.rows),
		yesvecs(vec), hessen(false)
	{
		/*// Allocate a
		a = new double*[n];
		for(int i=0; i<n; i++) a[i] = new double[n];
		// Copy aa to a
		for(int i=0; i<n; i++) for(int j=0; j<n; j++) a[i][j] = aa.at<double>(i,j);
		// Allocate zz
		zz = new double*[n];
		for(int i=0; i<n; i++) zz[i] = new double[n];
		// Initialize zz to zeros
		for(int i=0; i<n; i++) for(int j=0; j<n; j++) zz[i][j] = 0.0;
		// Allocate wri
		wri = new double[n];
		// Allocate scale
		scale = new double[n];
			...
			if (l == nn) {
				...
				if (l == nn-1) {
					...
					if (q >= 0.0) {
						...
						//cout << "complex" << endl;
						// only get real part
						wri[nn]=x+p;//Complex(x+p,-z);
						wri[nn-1]=wri[nn];//conj(wri[nn]);
					}
					nn -= 2;
				} else {
