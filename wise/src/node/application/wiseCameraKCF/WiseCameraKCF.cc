...
#include "WiseCameraKCF.h"
#include "TMacControlMessage_m.h"
#include "WiseCameraKCFMsg_m.h"

Define_Module(WiseCameraKCF);

#define LOGGER logger << "[" << simTime() << "] @ " << self << " : "
#define BASE_TRACE trace() << "WISEKCF: "

ofstream WiseCameraKCF::logger;
std::ofstream *final_writer=NULL;

/*! Class destructor */
WiseCameraKCF::~WiseCameraKCF()
{
    if (final_writer) {
        final_writer->close();
        delete final_writer;
...
void WiseCameraKCF::at_startup()
{
    ...
    os << "res/KCF" << "_r" << std::setfill('0') << std::setw(4) << _currentRun << "_N" << std::setfill('0') << std::setw(2) << n_nodes << "_T" << std::setfill('0') << std::setw(2) << n_targets << "_I" << std::setfill('0') << std::setw(2) << iter_max << ".dat";
    ...
    os1 << "res/KCF" << "_r" << std::setfill('0') << std::setw(4) << _currentRun << "_N" << std::setfill('0') << std::setw(2) << n_nodes << "_T" << std::setfill('0') << std::setw(2) << n_targets << "_I" << std::setfill('0') << std::setw(2) << iter_max
    ...
    os2 << "res/KCF" << "_r" << std::setfill('0') << std::setw(4) << _currentRun << "_N" << std::setfill('0') << std::setw(2) << n_nodes << "_T" << std::setfill('0') << std::setw(2) << n_targets << "_I" << std::setfill('0') << std::setw(2) << iter_max << ".rad";
    radioModule->initLogger(os2.str().c_str());

    LOGGER << "procNoiseCov=" << procNoiseCov << " measNoiseCov=" << measNoiseCov << " iter_max=" << iter_max << " alpha=" << alpha << endl;
}

/*! Read parameters of MTT_ICF*/
void WiseCameraKCF::readParameters()
{
...
void WiseCameraKCF::initStructures()
{
 ...
 */
void WiseCameraKCF::at_timer_fired(int index)
{
 ...
 */
void WiseCameraKCF::handleMacControlMessage(cMessage *c)
{
 ...
 */
void WiseCameraKCF::make_measurements(const vector<WiseTargetDetection>&dt)
{
    ...
    for (; tid < n_targets && tid < dt.size(); tid++)
    {
        ...
        LOGGER << "tid=" << tid << " -> MAKE MEASUREMENT" << " z=[" << z[tid].at<double>(0) << "," << z[tid].at<double>(0) << "]" << endl;
    }
    ...
    for (tid = 0; tid < n_targets; tid++) {
        ...
        //LOGGER << "tid="<< tid << " xgt=[" << xgt[tid].at<double>(0) << "," << xgt[tid].at<double>(1) << "]" << endl;
    }
}
 ...
 */
void WiseCameraKCF::at_tracker_init()
{
    ...
    for (unsigned tid = 0; tid < n_targets; tid++)
    {
        ...
        if (_share_msg.compare("FOV") == 0)
            ctrl.n_neighbourgs = neighborsFOV.size();//neighbors in the FOV graph
        else
            ctrl.n_neighbourgs = neighborsCOM.size();//neighbors in the COM graph
 ...
 */
bool WiseCameraKCF::at_tracker_first_sample()
{
	// We can:
...
//TODO
      // LOGGER << "tid=" << tid << " INIT x_=" << print(KF.x_,3) << " xgt=[" << xgt[tid].at<double>(0) << "," << xgt[tid].at<double>(1) << "]" << endl;
    }
 ...
 */
bool WiseCameraKCF::at_tracker_end_first_sample()
{
	// We can:
	// 1- either put here any first-sample specific code
	// 2- or simply treat the first sample as the others 
 ...
 */
bool WiseCameraKCF::at_tracker_sample()
{
 ...
 */
bool WiseCameraKCF::at_tracker_end_sample()
{
    //check if the camera is still collaborating
    if (_camStatus != kcf::INACTIVE) {
        LOGGER << "node not ready (own)" << endl;
    ...
    if (_dummy_comms==false) {
        ...
        if (i!= self) {
            ...
            WiseCameraKCF*c = check_and_cast<WiseCameraKCF*>(m);
            if (c->_camStatus != kcf::INACTIVE) {
                LOGGER << "node not ready (netcheck)" << endl;
                setTimer(ALARM_WAIT_NEGOTIATION,sampling_time); //we check again after a certain time
                return false;
            }
        }
   ...
   /*
    ...
    for (unsigned int tid = 0; tid < n_targets; tid++) {
        ...
        for (int i=0; i < n_nodes; i++)
            if (ctrl.nb_data_buffer[0].nb_data[i].end_collaboration == true){
                //LOGGER << "finished node=" << i << endl;
                c++;
            }
            else
                //LOGGER << "not finished node=" << i << endl;
    ...
    if (tid_c < n_targets || _camStatus != kcf::INACTIVE){
        LOGGER << "node not ready, tid=" << tid_c << " status=" << _camStatus << " c=" << c << " n=" << node_controls[0].n_neighbourgs << endl;
        setTimer(ALARM_WAIT_NEGOTIATION,sampling_time); //we check again after a certain time
        return false;
    }
    else {
 ...
 */
void WiseCameraKCF::estimateTarget(int tid)
{
 ...
 * @param[in] tid Target ID of the associated KF structures
 */
int WiseCameraKCF::sendKCFmessage(int tid)
{
    std::ostringstream os;
    //double delay=0.001;

    //create message & copy data
    os << "KCF: node=" << self << " tid=" << tid << " t=" << tracking_step_counter << " k=" << node_controls[tid].iter_counter;
    WiseCameraKCFMsg *pkt = new WiseCameraKCFMsg(os.str().c_str(), APPLICATION_PACKET);

    if (node_controls[tid].iter_counter == 1)
    {
        LOGGER << "tid=" << tid << " -> SENDING MSG to nodes for k=" << node_controls[tid].iter_counter << endl;
    ...
    else
    {
        if (node_controls[tid].iter_counter > iter_max){
            LOGGER << "tid=" << tid << " -> SENDING MSG to nodes to END consensus" << endl;
 ...
 *
 * @param[in] pkt Received packet (should be casted to a WiseCameraKCFMessage type)
 */
void WiseCameraKCF::process_network_message(WiseApplicationPacket *pkt)
{
    double e2=0,e1 = cv::getTickCount();
    int c = 0, tid = -1 , node = -1, i = 0, index = 0;

    WiseCameraKCFMsg *m = check_and_cast<WiseCameraKCFMsg*>(pkt);
    ApplicationInteractionControl_type ctl = m->getApplicationInteractionControl();
    node = atoi(ctl.source.c_str());
    tid = m->getTargetID();
    node_ctrl_t &ctrl = node_controls[tid]; //get the control structure associated to target 'tid'
    LOGGER << "RX from " << ctl.source << ". Packet content: [TID=" << m->getTargetID() << " t=" << m->getTrackingCount()  << " k="<< m->getIterationStep() << " Pos=(" << m->getPositionX() << "," << m->getPositionY() << ")]" << endl;
    ...
    switch (m->getPktType()){
        ...
        case KCF_COLLABORATION_DATA2:
            ...
            //compute consensus if we have all neighbor's data
              ...
              {
                  LOGGER << " CONSENSUS for k=" <<  ctrl.iter_counter << endl;
                  ...
                  LOGGER << " Waiting for " << node_controls[tid].n_neighbourgs-c << "/" << node_controls[tid].n_neighbourgs << "  nodes" << endl;

        break;
    case KCF_COLLABORATION_END:
        ctrl.nb_data_buffer[0].nb_data[node].end_collaboration=true;
        LOGGER << " COLLABORATION END RECEIVED from node=" << node << endl;
        break;
    }
 ...
 * @param[in] tid Target ID of the associated KF structures
 */
void WiseCameraKCF::consensusMesurements(int tid)
{
 ...
 * @param[in] tid Target ID of the associated KF structures
 */
void WiseCameraKCF::consensusState(int tid)
{
 ...
 */
void WiseCameraKCF::handleDirectApplicationMessage(WiseApplicationPacket *pkt)
{
    // Called when a DirectApplication Message is received
    LOGGER << "HANDLE DIRECT APPLICATION MESSAGE";
 ...
 * @param[in] pkt Received packet (should be a WiseApplicationPacket type)
 */
void WiseCameraKCF::logResult(int tid)
{
   ...
   //TODO
    //LOGGER << "tid=" << tid << " -> est:" << print(states_final[tid],2) << " Z:" << print(z[tid],2) << " gt:" << print(xgt[tid],2) << " for t="<< tracking_step_counter << endl;
    //*final_writer << std::setfill('0') << std::setw(5)  << simTime().dbl() << "\t" << tracking_step_counter << "\t" << self << "\t" << tid  << "\t"
    *final_writer << std::setw(5)  << simTime().dbl() << "\t" << tracking_step_counter << "\t" << self << "\t" << tid  << "\t"
 ...
 */
int WiseCameraKCF::findIndexInBuffer(int tid, int iter_index)
{
 ...
 *
 */
void WiseCameraKCF::storeDataInBuffer(int tid, int indBuf, int nodeID, WiseCameraKCFMsg *m)
{
    node_ctrl_t &ctrl = node_controls[tid]; //control for target
