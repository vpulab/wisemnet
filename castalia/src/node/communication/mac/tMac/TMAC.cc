	...
	switch (timer) {
		...
		case FRAME_START:{
			...
			for (int i = 1; i < (int)scheduleTable.size(); i++) {
			    ...
			    //JCS PROBLEM: offset may be negative for small values of 'frameTime'
			    //JCS SOLUTION: change 'if' to 'while'
				//if (scheduleTable[i].offset < 0) {
			    while (scheduleTable[i].offset.dbl() < 0) {
	                //printf(" WAKEUP_SILENT=%.8f\n",scheduleTable[i].offset.dbl());
	            ...
	            //setTimer(WAKEUP_SILENT + i, scheduleTable[i].offset+frameTime);
			}
...
void TMAC::resetDefaultState(const char *descr)
{
	...
	if (activationTimeout <= getClock()) {
		...
		while (!TXBuffer.empty()) {
			if (txRetries <= 0) {
				collectOutput("WMSN failures", "Max tx retries");
				TMacControlMessage *stats;
				stats = new TMacControlMessage("Mac Notify Stats", MAC_CONTROL_MESSAGE);
				stats->setMaxTxRetries(maxTxRetries);
				stats->setTxRetries(maxTxRetries);
				stats->setType(TMAC_TX_FAILURE);
				toNetworkLayer(stats);
...
{
//cerr << "caller2  @ " << self << " " << simTime() << "  : " << frameTime << endl;
	updateScheduleTable(frameTime, self, 0);
	//printf(" resyncTime=%.8f\n",resyncTime);
	setTimer(SYNC_RENEW, resyncTime);
}
...
//cerr << "TEST @ " << self << " " << simTime() << "  : " << wakeup << endl;
...
if (wakeup < 0) {
//cerr << "MERDA @ " << self << " " << simTime() << " w = " << wakeup << "  now setting to 0" << endl;
wakeup = 0;
}
                    //printf(" wakeup2=%.8f\n",wakeup.dbl());
	...
	if (currentFrameStart == -1) {
		...
		//printf(" currentFrameStart=%.8f\n",currentFrameStart.dbl());
...
void TMAC::fromRadioLayer(cPacket * pkt, double RSSI, double LQI)
{
	...
	switch (macPkt->getType()) {
		...
		case RTS_TMAC_PACKET:{
			...
			//printf(" TX_TIME(ctsPacketSize) + waitTimeout=%.8f\n",TX_TIME(ctsPacketSize) + waitTimeout.dbl());
		...
		case ACK_TMAC_PACKET:{
			if (macState == MAC_STATE_WAIT_FOR_ACK && source == txAddr) {
				...
				stats = new TMacControlMessage("Mac Notify Stats", MAC_CONTROL_MESSAGE);
				stats->setMaxTxRetries(maxTxRetries);
				stats->setTxRetries(maxTxRetries - txRetries);
				stats->setType(TMAC_TX_SUCCESS);
				toNetworkLayer(stats);
				cancelTimer(TRANSMISSION_TIMEOUT);
				popTxBuffer();
...
//cerr << "caller  @ " << self << " " << simTime() << "  : " << macPkt->getSync() << endl;
