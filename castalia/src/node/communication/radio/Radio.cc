...
void Radio::initLogger(std::string filename){
    ...
    if (self == 0)
    if (!_logger->is_open()){
           _logger->open(_filename.c_str());
           *_logger << "#simulation results for radio packets" << endl;
                   *_logger << "#Ptx  : Packets transmitted" << endl;
                   *_logger << "#RxRNI: RxReachedNoInterference" << endl;
                   *_logger << "#RxRI : RxReachedInterference" << endl;
                   *_logger << "#RxFNI: RxFailedNoInterference" << endl;
                   *_logger << "#RxFI : RxFailedInterference" << endl;
                   *_logger << "#RxFS : RxFailedSensitivity" << endl;
                   *_logger << "#RxFM : RxFailedModulation" << endl;
                   *_logger << "#RxNRx: RxFailedNoRxState" << endl;
                   *_logger << "#bufO : BufferOverflow" << endl;
           *_logger << "#" << "node\tPtx\tRxRNI\tRxRI\tRxFNI\tRxFI\tRxFS\tRxFM\tRxNRx\tbufO" << endl;
           _logger->close();
    }
}
void Radio::finishSpecific()
{
	...
	if (stats.bufferOverflow > 0)
	 ...
	 if (_logger != NULL) {
	     _logger->open(_filename.c_str(),fstream::app);
	     *_logger << self << "\t" << stats.transmissions << "\t" << stats.RxReachedNoInterference << "\t" << stats.RxReachedInterference << "\t"
	                 << stats.RxFailedNoInterference << "\t" << stats.RxFailedInterference << "\t"
	                 << stats.RxFailedSensitivity << "\t" << stats.RxFailedModulation << "\t"
	                 << stats.RxFailedNoRxState << "\t" << stats.bufferOverflow << endl;
	     _logger->close();
	 }
}
...
RXmode_type *Radio::getRXmode()
{
    return &*RXmode;
}

TxLevel_type *Radio::getTXlevel()
{
    return &*TxLevel;
}

SleepLevel_type *Radio::getSleeplevel()
{
    return &*sleepLevel;
}
