...
void VirtualApplication::initialize()
{
	...
	if (parent->findSubmodule("ResourceManager") != -1) {
	    ...
	    resMgrModule = check_and_cast <WiseResourceManager*>(parent->getSubmodule("ResourceManager"));
	} else {
...
void VirtualApplication::requestSensorReading(int index)
{
	...
	SensorReadingMessage *reqMsg = new SensorReadingMessage("app 2 sensor dev manager (Sample request)", SENSOR_READING_MESSAGE);
